...
object BodyParts {
    ...
    fun displayList(
        parts: List<String>,
        separator: String = " · ",
    ): String = parts.joinToString(separator) { display(it) }

    /** All unique body parts present in the catalog, sorted. */
    fun distinctSorted(stretches: List<Stretch>): List<String> = stretches.flatMap { it.bodyParts }.distinct().sorted()

    /** Same as [distinctSorted] but with "all" prepended for filter UIs. */
    fun filterOptions(stretches: List<Stretch>): List<String> = listOf(ALL) + distinctSorted(stretches)
