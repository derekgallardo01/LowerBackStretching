...
class SessionDaoTest {
    ...
    @Test
    fun recent_returns_in_descending_order_and_respects_limit() =
        runBlocking {
            for (i in 1..5) {
                dao.insert(
    ...
    @Test
    fun completedDays_returns_distinct_days_descending() =
        runBlocking {
    ...
    @Test
    fun forDay_filters_by_epochDay() =
        runBlocking {
            dao.insert(session(programId = "p", day = 1, epochDay = 100L, millis = 1L, duration = 60))
