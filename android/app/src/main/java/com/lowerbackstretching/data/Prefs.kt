...
class Prefs(
    ...
    val reminderMinute: Flow<Int> = context.dataStore.data.map {
        it[PrefKeys.REMINDER_MINUTE] ?: ReminderDefaults.MINUTE
    }
    ...
    val durationUnit: Flow<DurationUnit> = context.dataStore.data.map {
        DurationUnit.fromStorage(it[PrefKeys.DURATION_UNIT])
    }
    ...
    val musicVolume: Flow<Float> = context.dataStore.data.map {
        it[PrefKeys.MUSIC_VOLUME] ?: AudioDefaults.MUSIC_VOLUME
    }
    val ambientTrack: Flow<AmbientTrack> = context.dataStore.data.map {
        AmbientTrack.fromStorage(it[PrefKeys.AMBIENT_TRACK])
    }
    val ambientVolume: Flow<Float> = context.dataStore.data.map {
        it[PrefKeys.AMBIENT_VOLUME]
            ?: AudioDefaults.AMBIENT_VOLUME
    }
    ...
    internal suspend fun setReminder(
        enabled: Boolean,
        hour: Int,
        minute: Int,
    ) {
