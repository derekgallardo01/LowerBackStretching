...
class ProgramProgressRepositoryTest {
    @Test
    fun `currentDay defaults to 1 when no record exists`() =
        runTest {
    ...
    @Test
    fun `advance writes nextDay one past the completed day`() =
        runTest {
            val dao = FakeDao()
    ...
    @Test
    fun `advance caps at totalDays plus one`() =
        runTest {
            val dao = FakeDao()
            val repo = ProgramProgressRepository(dao)
    ...
    @Test
    fun `advance ignores synthetic single ids`() =
        runTest {
            val dao = FakeDao()
            val repo = ProgramProgressRepository(dao)
    ...
    @Test
    fun `advance ignores synthetic routine ids`() =
        runTest {
            val dao = FakeDao()
            val repo = ProgramProgressRepository(dao)
