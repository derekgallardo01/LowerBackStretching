...
class PainLogRepositoryTest {
    @Test
    fun `recordPre writes a row tagged PRE_SESSION with null sessionId`() =
        runTest {
            val dao = FakeDao()
    ...
    @Test
    fun `recordPost writes a row tagged POST_SESSION linked to a session`() =
        runTest {
            val dao = FakeDao()
            val repo = PainLogRepository(dao)
    ...
    @Test
    fun `hasPreLoggedToday returns false on a clean store`() =
        runTest {
            val dao = FakeDao()
            val repo = PainLogRepository(dao)
    ...
    @Test
    fun `hasPreLoggedToday returns true after recordPre on the same day`() =
        runTest {
            val dao = FakeDao()
            val repo = PainLogRepository(dao)
    ...
    @Test
    fun `hasPreLoggedToday queries only PRE_SESSION rows`() =
        runTest {
            val dao = FakeDao()
    ...
    @Test
    fun `hasPreLoggedToday respects the start-of-day boundary`() =
        runTest {
            val dao = FakeDao()
            // A PRE log from yesterday must NOT count as today.
            val yesterdayMidday = LocalDate
                .now()
                .minusDays(1)
                .atTime(12, 0)
            ...
            dao.inserted += PainLogEntity(
                id = 1,
                recordedAtEpochMillis = yesterdayMidday,
                painLevel = 5,
                bodyLocationTag = null,
    ...
    @Test
    fun `delete removes the row from the flow`() =
        runTest {
            val dao = FakeDao()
            val repo = PainLogRepository(dao)
    ...
    private class FakeDao : PainLogDao {
        ...
        override fun latest(): Flow<PainLogEntity?> = rows.map { list -> list.maxByOrNull { it.recordedAtEpochMillis } }
