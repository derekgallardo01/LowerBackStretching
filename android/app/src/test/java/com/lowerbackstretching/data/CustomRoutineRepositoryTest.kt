...
class CustomRoutineRepositoryTest {
    @Test
    fun `create joins stretchIds with comma and trims name`() =
        runTest {
            val dao = FakeDao()
    ...
    @Test
    fun `create empty stretches saves empty csv`() =
        runTest {
            val dao = FakeDao()
            val repo = CustomRoutineRepository(dao)
    ...
    @Test
    fun `update rewrites name and stretches`() =
        runTest {
            val dao = FakeDao()
            val repo = CustomRoutineRepository(dao)
    ...
    @Test
    fun `duplicate inserts a copy with suffix and zero id`() =
        runTest {
            val dao = FakeDao()
            val repo = CustomRoutineRepository(dao)
    ...
    @Test
    fun `reorder assigns sequential displayOrder values`() =
        runTest {
            val dao = FakeDao()
            val repo = CustomRoutineRepository(dao)
    ...
    @Test
    fun `softDelete sets deletedAt to non-null`() =
        runTest {
            val dao = FakeDao()
            val repo = CustomRoutineRepository(dao)
    ...
    private class FakeDao : CustomRoutineDao {
        ...
        override suspend fun update(routine: CustomRoutineEntity) {
            lastUpdated = routine
        }

        override suspend fun delete(routine: CustomRoutineEntity) {
            lastDeleted = routine
        }
        ...
        override suspend fun setDisplayOrder(
            id: Long,
            order: Int,
        ) {
        ...
        override suspend fun setDeletedAt(
            id: Long,
            deletedAt: Long?,
        ) {
