...
class NoopSyncBackendTest {
    ...
    @Test fun `signedInUid returns null`() =
        runTest {
            assertThat(backend.signedInUid()).isNull()
        }

    @Test fun `signInAnonymously returns null`() =
        runTest {
            assertThat(backend.signInAnonymously()).isNull()
        }

    @Test fun `signOut completes without error`() =
        runTest {
            backend.signOut() // shouldn't throw
        }

    @Test fun `pushSession returns false`() =
        runTest {
    ...
    @Test fun `pushRoutine returns false`() =
        runTest {
    ...
    @Test fun `pushProgramProgress returns false`() =
        runTest {
            assertThat(backend.pushProgramProgress("p1", 2, 0L)).isFalse()
        }

    @Test fun `pushFlexibilityTest returns false`() =
        runTest {
